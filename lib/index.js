 ...
 */ 
function getBorderColor(borderColor){
	var color;
	if (borderColor){
    if (borderColor(0,1) == '#'){
      color = borderColor.replace('#', '');
    }else if (borderColor(0,3) == 'rgb'){  
      color = borderColor.replace(',', '-');
      if (borderColor(0,3) == 'rgba'){
        var colorParams = borderColor.split('-'),
        	opacity = parseInt(parseFloat(colorParams[colorParams.length -1])*100);
...
function getImageTagSource(path, options) {
	var tokens = path.split('/'),
		...
		borderColor = getBorderColor(options.borderColor);
	if (options){
		if (options.width){
