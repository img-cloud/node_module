...
function validateTransform(path, options){
	...
	if (options){
		...
		if (typeof options.x !== 'undefined' && (options.x === null || options.x === 'undefined')){
			return 'Invalid value for x. Must be a Number';
		}
		if (typeof options.y !== 'undefined' && (options.y === null || options.y === 'undefined')){
			return 'Invalid value for y. Must be a Number';
		}
...
function getImageTagSource(path, options) {
	...
	if (options){
		if (options.width){
			transformationCsv += 'w_' + options.width;
		}
		if (options.height){
			transformationCsv += 'h_' + options.height;
		}
		if (typeof options.crop !== 'undefined'){
			transformationCsv += 'crop_' + String(options.crop);
		}
		if (options.x){
			transformationCsv += 'x_' + options.x;
		}
		if (options.y){
			transformationCsv += 'y_' + options.y;
		}
		if (typeof options.scale !== 'undefined'){
			transformationCsv += 'scale_' + String(options.scale);
		}
		if (options.borderColor || options.borderWidth){
			transformationCsv += 'border_';
			if(options.borderColor){
				transformationCsv += options.borderColor;
			}
			transformationCsv += '-';
			if(options.borderWidth){
				transformationCsv += options.borderWidth;
			}
		}
		if (options.blur){
			transformationCsv += 'blur_' + options.blur;
		}
		if (options.format){
 ...
 *  options.height, options.x and options.y. If x and y are not specified, the original image dimensions are used
 * @param {Number} [options.x] Used with options.crop
 * @param {Number} [options.y] Used with options.crop
